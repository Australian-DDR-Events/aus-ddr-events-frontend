...
const Register = () => {
  ...
  const { loggedInUser, register } = useAuthentication();

  useEffect(() => {
    if (loggedInUser?.token) setSignupLoading(false);
  }, [loggedInUser]);

  const [newDancerResult, addNewDancer] = useAddNewDancerMutation();
  const [dancerInput, setDancerInput] =
    useState<AddDancerInput | undefined>(undefined);
  ...
  const onSubmit = (
    ...
    register(values.email, values.password).then((authResult) => {
      if (authResult.isOk()) {
        setDancerInput({
